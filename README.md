...
**Frame rates are reported as skipped, never invented.** A tab that is not compositing parks `requestAnimationFrame`, and any measurement would return zero. The same rule is why the full Electron self-check (`npm run selftest`, 49 + 31 checks in the real shell) is run locally with the window in front, while CI runs the 32 shell unit tests, which do not depend on a screen. Since 1.1.0 the window opens where it was closed and at the same size (`window-state.json` in the app's data folder); if that monitor is gone, it opens in the middle of the main one.
